...
# Set to False if we detect that macostools.copy() does not work (fails on 64-bit MacOS)
        ...
        if options.link:
            ...
            if _supports_macostools:
                try:
                    ...
                    print >> sys.stderr, "macostools.copy() on this system, reverting to shutil.copy2()"
...
class ExportFile(object):
        ...
        try:
            if os.path.exists(self.export_file):
                if os.path.getmtime(self.export_file) < os.path.getmtime(source_file):
                    print ('Will update %s because a newer version is available: %s vs. %s' %
                           (self.export_file, time.ctime(os.path.getmtime(self.export_file)), 
                    ...
                    if diff > _MAX_FILE_DIFF or (diff > 32 and options.link):
                        print ('Will update %s because of file size: %d vs. %d' %
            ...
            if options.originals and self.photo.originalpath is not None:
                ...
                if os.path.exists(self.original_export_file):
                    if (os.path.getmtime(self.original_export_file) <
                        ...
                        print ('Will update %s because a newer version is available: %s vs. %s' %
                               (self.original_export_file, time.ctime(os.path.getmtime(self.original_export_file)), 
                        ...
                        if diff > _MAX_FILE_DIFF or (diff > 0 and options.link):
                            print ('Will update %s because of file size: %d vs. %d' %
                                   (self.original_export_file, export_size, source_size))
                            do_original_export = True
                else:
                    do_original_export = True

                do_iptc = (options.iptc == 1 and do_original_export) or options.iptc == 2
                if do_iptc and options.link:
                    self.check_iptc_data(original_source_file, options, is_original=True)
                if do_original_export:
                    copy_or_link_file(original_source_file, self.original_export_file,
                                      options)
                if do_iptc and not options.link:
                    self.check_iptc_data(self.original_export_file, options, is_original=True)
    ...
    def region_matches(self, region1, region2):
        ...
        for i in xrange(len(region1)):
            if abs(region1[i] - region2[i]) > 0.0000001:
                print "value %d %f %f %f" % (i, region1[i], region2[i], region1[i] - region2[i])
                return False
        return True
    ...
    def check_iptc_data(self, export_file, options, is_original=False):
        ...
        (file_keywords, file_caption, date_time_original, rating,
            gps, region_rectangles, region_names) = exiftool.get_iptc_data(export_file)
        ...
        if options.faces:
            ...
            if combined_region_names != combined_photo_faces:
                print 'Updating IPTC for %s because of persons (%s instead of %s)' % (
                    su.fsenc(export_file), su.fsenc(combined_region_names),
                    su.fsenc(combined_photo_faces))
                persons_diff = True
            else:
                for p in xrange(len(region_rectangles)):
                    if not self.region_matches(region_rectangles[p], photo_rectangles[p]):
                        print 'Updating IPTC for %s because of region for %s (%s vs %s)' % (
                            su.fsenc(export_file),
                            region_names[p], ','.join(str(c) for c in region_rectangles[p]),
        ...
        if new_caption or new_keywords != None or new_date or new_gps or new_rating != -1 or persons_diff:
            if not options.dryrun:
                exiftool.update_iptcdata(export_file, new_caption, new_keywords, 
                                         new_date, new_rating, new_gps, new_rectangles,
                                         new_persons)
            return True
        return False
...
class ExportDirectory(object):
    ...
    def make_album_basename(self, orig_basename, index, name_template):
        ...
        orig_basename = re.sub(re.compile(r'\.(jpeg|jpg|mpg|mpeg|mov|png|tif|tiff)$',
    ...
    def scan_originals(self, folder, options):
        ...
        for f in file_list:
            ...
            # everything else must have a master, or will have to go
            if not master_file or originalfile != master_file.original_export_file:
...
def export_iphoto(data, export_dir, excludes, exclude_folders, options):
    ...
    if options.facealbums is not None:
        album.process_albums(data.getfacealbums(), ["Face"], options.facealbums, 
                             ".", excludes, options)
...
def parseArgs():
    global parser  # TODO: make this not global
    ...
    parser.add_option(
        ...
        help="Show what would have been done, but don't change or copy any files.")
    parser.add_option(
        "-e", "--events", 
    ...
    parser.add_option("--export",
                      ...
                      Any files found in this folder that are not part of the export set will be
      deleted, and files that match will be overwritten if the iPhoto version
      of the file is different.""")
    parser.add_option("--facealbums",
                      help="""Create albums (folders) for faces, using the argument as a folder name prefix.""")
    parser.add_option("--face_keywords", action="store_true",
    ...
    parser.add_option("--iphoto",
                      help='Path to iPhoto library, e.g. "~/Pictures/iPhoto Library".',
                      default=os.environ.get('HOME') + "/Pictures/iPhoto Library")
    parser.add_option(
    ...
    parser.add_option("--picasa", action="store_true",
...
def main():
    ...
    if options.size and options.faces:
        # TODO: and why not?
        parser.error("Cannot use --size and --faces together.")
    ...
    album_xml_file = iphotodata.get_album_xmlfile(options.iphoto)
    ...
    if options.export:
        if not options.albums and not options.events and not options.smarts:
            parser.error("Need to specify at least one event, album, or smart album" +
                         "for exporting, using the -e, -a, or -s options.")
        export_iphoto(data, options.export, options.exclude, exclude_folders, options)
    else:
        print >> sys.stderr, "No action specified. Use --export to export from your iPhoto library."
        return 1

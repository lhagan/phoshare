        ...
        try:
            if os.path.exists(self.export_file):
                if (os.path.getmtime(self.export_file) < 
                    ...
                    print ('Will update %s because a newer version is '
                           'available: %s vs. %s' %
                           (self.export_file, 
                ...
                if os.path.exists(self.original_export_file):
                    if (os.path.getmtime(self.original_export_file) <
                        ...
                        if diff > _MAX_FILE_DIFF or (diff > 0 and options.link):
                            print ('Will update %s because of file size: %d '
                                   'vs. %d' %
                                   (self.original_export_file, export_size, 
                                    source_size))
                            do_original_export = True
                else:
                    do_original_export = True
...
def getOptionParser():
    ...
    p.add_option("--export",
                 help="""Export images and movies to specified folder.
                      ...
                      different. d""")
    p.add_option("--facealbums",
                 help="""Create albums (folders) for faces, using the 
                      argument as a folder name prefix.""")
...
def main():
    ...
    if options.export:
        album = ExportLibrary(os.path.join(options.export))
